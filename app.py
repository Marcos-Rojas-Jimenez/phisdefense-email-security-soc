...
def collect_external_reports_status():
    ...
    dmarc_paths = [
        "/opt/tfm-evidencias/resultados/dmarc_reportes_reales",
        "/opt/tfm-evidencias/resultados/dmarc",
        "/opt/tfm-evidencias/reports/dmarc",
        "/opt/dmarc-analyzer/output",
    ]
    ...
    dmarc_summary_candidates = [
        "/opt/dmarc-analyzer/output/resumen.txt",
        "/opt/dmarc-analyzer/output/resumen_dmarc.txt",
        "/opt/tfm-evidencias/resultados/dmarc_reportes_reales/resumen.txt",
        "/opt/tfm-evidencias/resultados/dmarc_reportes_reales/resumen_dmarc.txt",
    ]
    ...
    if not dmarc_summary:
        if dmarc_files:
            ...
            dmarc_summary = (
                "No se han localizado informes DMARC reales de Google en las rutas de evidencias esperadas.\\n"
                "Rutas revisadas:\\n"
                + "\\n".join(dmarc_paths)
            )

    # TLS-RPT real: NO contar txt como reportes JSON.
    tls_base = Path("/opt/tfm-evidencias/resultados/tls-rpt-real")
...
def external_reports_panel():
    ...
    dmarc_last = str(status["dmarc_files"][0]) if status["dmarc_files"] else "Sin informes DMARC reales detectados"
    ...
    tls_last = str(status["tls_json_files"][0]) if status["tls_json_files"] else "Sin reportes TLS-RPT JSON parseables"

    return html.Section(
        [
            page_heading(
                "Informes externos",
                "Reportes reales DMARC de Google y revisión TLS-RPT",
                "Este bloque muestra solo evidencias reales de reporting externo; no incluye CSV internos de estado, MTA-STS ni servicios.",
            ),
            dbc.Row(
                [
                    dbc.Col(
                        dbc.Card(
                            dbc.CardBody(
                                [
                                    ...
                                    html.Div("evidencias DMARC reales localizadas", className="external-report-subtitle"),
                        ...
                        dbc.Card(
                            dbc.CardBody(
                                [
                                    ...
                                    html.Div("reportes JSON parseables localizados", className="external-report-subtitle"),
